...
export default function MF({ onSaved, defaultValues, disabled }) {
  const getTodayDate = () => new Date().toISOString().split('T')[0];
  const [f, sf] = useState({
    weightKg: '',
    heightCm: defaultValues?.heightCm || 175,
  ...
  useEffect(() => {
    if (defaultValues) {
      sf(prev => ({
        ...prev,
        heightCm: defaultValues.heightCm || prev.heightCm,
        age: defaultValues.age || prev.age,
        sex: defaultValues.sex || prev.sex,
        activity: defaultValues.activity || prev.activity,
      }));
    }
  }, [defaultValues]);
  ...
  if (disabled) {
    return (
      <div className="alert alert-error" style={{ margin: 0 }}>
        Please complete your profile first before logging measurements.
      </div>
    );
  }
  ...
  const sub = async e => {
    ...
    try {
      ...
      setSuccess(true);
      sf(prev => ({ ...prev, weightKg: '', measurementDate: getTodayDate() }));
  ...
  const hasProfile = !!defaultValues;
    ...
    <form onSubmit={sub}>
      ...
      {hasProfile && (
        <div className="profile-info-row">
          <span>Using profile: <strong>{f.heightCm} cm</strong> · <strong>{f.age} yrs</strong> · <strong>{f.sex}</strong> · <strong>{f.activity}</strong></span>
        </div>
      )}
