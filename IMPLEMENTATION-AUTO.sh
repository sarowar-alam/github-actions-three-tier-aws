...
#
#  CI / GitHub Actions (self-hosted runner — checkout already done):
#    sudo ./IMPLEMENTATION-AUTO.sh -d domain -e email --skip-pull
#    → Same as re-deploy but skips the internal git pull; the runner's
#      actions/checkout step already placed the correct commit on disk
#
#  Options:
#    -d | --domain     Your domain name (must resolve to this server's IP)
#    -e | --email      Email for Let's Encrypt registration (required with -d)
#    -s | --skip-pull  Skip internal git pull (used by CI pipelines)
...
usage() {
  echo "Usage: sudo $0 [-d domain.com -e admin@domain.com] [-s]"
  echo "  -d | --domain     Domain name for Nginx + SSL"
  echo "  -e | --email      Email for Let's Encrypt (required with -d)"
  echo "  -s | --skip-pull  Skip internal git pull (used by CI pipelines)"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -s|--skip-pull)  SKIP_PULL=true;  shift   ;;
    -h|--help)       usage ;;
...
if [[ "$REDEPLOY" == true ]]; then
  if [[ "$SKIP_PULL" == true ]]; then
    info "Mode       : RE-DEPLOY  (CI — checkout already done, skipping git pull)"
  else
...
# Skipped when --skip-pull / -s is set: CI pipelines (e.g. GitHub Actions)
# run actions/checkout before invoking this script, so the workspace already
# contains the correct commit and a second pull is redundant.
if [[ "$REDEPLOY" == true && "$SKIP_PULL" == false ]]; then
...
elif [[ "$REDEPLOY" == true && "$SKIP_PULL" == true ]]; then
  step "Git pull — skipped (CI mode)"
  ok "Workspace at commit $(git -C "$SCRIPT_DIR" rev-parse --short HEAD 2>/dev/null || echo 'unknown')"
fi
...
if [[ "$USE_SSL" == true ]]; then
  ...
  # AWS EC2 does not support hairpin NAT — curl to the public domain from inside
  # the instance would hit the public IP and be refused. Force loopback instead.
...
# shellcheck disable=SC2086  # CURL_RESOLVE is intentionally unquoted (may be empty)
HTTP_STATUS=$(curl -sk $CURL_RESOLVE -o /dev/null -w "%{http_code}" "$HEALTH_URL" 2>/dev/null || true)
